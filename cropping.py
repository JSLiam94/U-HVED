...
def main(in_path, out_path, pat_category_list=('BraTS-GLI-'), crop=False):
    pattern = re.compile(r'BraTS-GLI-(\d+)-(\d+)')  # 正则表达式匹配文件名

    # 遍历主目录下的所有子目录
    for pat_folder_name in os.listdir(in_path):
        # 拼接完整的路径
        pat_folder = os.path.join(in_path, pat_folder_name)

        # 检查是否为目录
        if os.path.isdir(pat_folder):
            match = pattern.match(pat_folder_name)
            if match:
                pat_ID = match.group(1)  # 提取病人ID部分
                pat_cat = match.group(2)  # 可能用到的扫描序号或其他信息
                ...
                print(f"Skipping invalid folder name: {pat_folder_name}")
            ...
            pat_name = 'BraTS-GLI-%s-%s' % (pat_cat, pat_ID)
